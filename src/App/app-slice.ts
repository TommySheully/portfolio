...
export type MessageType = { name: string | null, email: string | null, message: string | null }
...
export const sendMessageTC = createAsyncThunk('app/getMeAuth', async (messageData: MessageType, { dispatch }) => {
    ...
    try {
        const { data } = await axios.post('https://sheet.best/api/sheets/c13c1917-32f4-494a-bcee-2efa8a857eee', {name, email, message})
